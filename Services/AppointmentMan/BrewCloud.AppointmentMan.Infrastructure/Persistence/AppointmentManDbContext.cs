using BrewCloud.AppointmentMan.Domain.Entities;
...
namespace BrewCloud.AppointmentMan.Infrastructure.Persistence
{
    public class AppointmentManDbContext  : DbContext
    {
        private Tenant _tenant;
        private string _historyTable;
        private readonly IIdentityRepository _identityRepository;
        public AccountInfoDto _accountInfo { get; set; }
        public AppointmentManDbContext(DbContextOptions<AppointmentManDbContext> options, ITenantRepository tenantRepository, IIdentityRepository identityRepository, string historyTable = "") : base(options)
        {
        ...
        public virtual DbSet<AppointmentManLogs> AppointmentManLogs { get; set; } 
        ...
        {
            List<AppointmentManLogs> logs = new List<AppointmentManLogs>();
            ...
            foreach (var entity in entities.Where(x => x.State == EntityState.Modified))
            {
                try
                {
                    ...
                    foreach (var property in entity.CurrentValues.Properties)
                    {
                        ...
                        {
                            logs.Add(new AppointmentManLogs
                            {
                                Id = Guid.NewGuid(),
                                Date = logDate,
            ...
            AppointmentManLogs.AddRangeAsync(logs); 
        }
    }
}
