...
namespace BrewCloud.AppointmentMan.Infrastructure.Repositories
{
    public class UnitOfWork : IUnitOfWork
    {
        protected AppointmentManDbContext _dbContext;
        private IDbContextTransaction _trans;
        public UnitOfWork(AppointmentManDbContext dbContext)
        {
        ...
        {
            var builder = new DbContextOptionsBuilder<AppointmentManDbContext>();
            builder.UseSqlServer(connectionString);
            using (var db = new AppointmentManDbContext(builder.Options, null, null))
            {
        ...
        public async Task MigrateDatabase(string connectionString, string targetMigrationName, string historyTable)
        {
            var builder = new DbContextOptionsBuilder<AppointmentManDbContext>();
            builder.UseSqlServer(connectionString);
            using (var db = new AppointmentManDbContext(builder.Options, null, null, historyTable))
            {
