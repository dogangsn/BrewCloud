...
using BrewCloud.Farm.Domain.Contracts;
using BrewCloud.Farm.Infrastructure.Persistence;
using System.Data;
...
namespace BrewCloud.Farm.Infrastructure.Repositories
{
    ...
    {
        protected FarmDbContext _dbContext;
        private IDbContextTransaction _trans;
        public UnitOfWork(FarmDbContext dbContext)
        {
        ...
        public async Task MigrateDatabase(string connectionString)
        {
            ...
            using (var db = new FarmDbContext(builder.Options, null))
            {
        ...
        public async Task MigrateDatabase(string connectionString, string targetMigrationName, string historyTable)
        {
            var builder = new DbContextOptionsBuilder<FarmDbContext>();
            builder.UseSqlServer(connectionString);
            using (var db = new FarmDbContext(builder.Options, null, historyTable))
            {
